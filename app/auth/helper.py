    ...
    def decorated_function(*args, **kwargs):
        ...
        if 'Authorization' in request.headers:
            ...
            try:
                ...
                return make_response(jsonify({'status': 'failed', 'message': 'Provide a valid auth token'})), 403
...
def response_auth(status, message, token, status_code):
    ...
    """
    return make_response(jsonify({'status': status, 'message': message, 'auth_token': token})), status_code
